...
; - The LPF commands need at least need a delay of 55 milliseconds
;   between commands. Sending faster then 55ms between commands will cause a
;   lock up. - May be caused the i2c RC constant: bus capacitance and pull up
;   resistance. The bus capacitance for the i2c gpio chips on the prototype is
;   much higher than the bus capacitance to the mux chips.
;
; Serial Commands
; ---------------
; ==Input / Output
...
; == Gain Commands
; X       - Gain Commands
; 1,2,3,4 - Select Output
; One bit - Hex Base Value for Pot Value.
; Example: G2] - Set the gain of output R to a value of 93, 0x53
;
; == Low Pass Filter Commands
; Z       - Gain Commands
; 1,2,3,4 - Select Output
; One bit - Hex Base Value for LPF Value.
; Example: F2] - Set the gain of output R to a value of 93, 0x53
;
    ...
    extern		Dis1Name,Dis2Name,Dis3Name,Dis4Name
    extern		DisInName,DisOutInDiv,DisPortDiv,SerComma
    extern		LCDinit,LCDaddr,LCDletr
    extern		SerDis1Name,SerDis2Name,SerDis3Name,SerDis4Name
...
; Manifest Constants -------------------------------------------------
PB_1		equ	H'00'	; PORTA pin for PB_1
PB_2		equ	H'01'	; PORTA pin for PB_2
PB_3		equ	H'02'	; PORTA pin for PB_3
PB_4		equ	H'03'	; PORTA pin for PB_4
...
PB_1PF_BIT	equ	H'01'	; PORTC pin for LFP and DOWN
...
EE_1		equ	H'00'	; EEPROM Locations
EE_2		equ	H'01'
EE_3		equ	H'02'
EE_4		equ	H'03'
...
Received1	equ	H'00'	; Serial command mode status flag bits.
Received2	equ	H'01'
Received3	equ	H'02'
Received4	equ	H'03'
ReceivedC	equ	H'04'
...
Received_Gain_1	equ	H'00'	; Gain LPF Serial command mode status flag bits.
Received_Gain_2	equ	H'01'
Received_Gain_3	equ	H'02'
Received_Gain_4	equ	H'03'
Received_LPF_1	equ	H'04'
Received_LPF_2	equ	H'05'
Received_LPF_3	equ	H'06'
Received_LPF_4	equ	H'07'
...
;AD5337 DAC for VCA control
AD5337_W1_Addr equ	B'00011010'
AD5337_W2_Addr equ	B'00011000'
AD5337_R1_Addr equ	B'00011011'
AD5337_R2_Addr equ	B'00011001'

;removed digtial pot stuff
...
; File Registers ------------------------------------------------------
    udata
Port1Count	res	1	; Which input that is connected to the out port.
Port2Count	res	1
Port3Count	res	1
Port4Count	res	1
...
Gain_1_Value	res	1	; VCA gain values.
Gain_2_Value	res	1
Gain_3_Value	res	1
Gain_4_Value	res	1

LPF_1_Value	res	1	; The LPF is selected.
LPF_2_Value	res	1
LPF_3_Value	res	1
LPF_4_Value	res	1
...
DATAEE    	org  		0xF000
    ...
    data	H'37',H'37',H'37',H'37' ; DAC Values
...
Start
;    pagesel	SIM		; For user with Simulator to goto "SIM"
;    goto	SIM		; label.
    ...
    call 	EEREAD		; Get Port Count from EEPROM
    banksel	Port3Count
    movwf 	Port3Count	; Store Port count

    call 	EEREAD		; Get Port Count from EEPROM
    banksel	Port4Count
    movwf 	Port4Count	; Store Port count

    call 	EEREAD		; Get Pot Value from EEPROM
    banksel	Gain_1_Value
    movwf 	Gain_1_Value	; Store Potentiometer Value

    call 	EEREAD		; Get Pot Value from EEPROM
    banksel	Gain_2_Value
    movwf 	Gain_2_Value	; Store Potentiometer Value

    call 	EEREAD		; Get Pot Value from EEPROM
    banksel	Gain_3_Value
    movwf 	Gain_3_Value	; Store Potentiometer Value

    call 	EEREAD		; Get Pot Value from EEPROM
    banksel	Gain_4_Value
    movwf 	Gain_4_Value	; Store Potentiometer Value

    call 	EEREAD		; Get LPF Value from EEPROM
    banksel	LPF_1_Value
    movwf 	LPF_1_Value	; Store Potentiometer Value

    call 	EEREAD		; Get LPF Value from EEPROM
    banksel	LPF_2_Value
    movwf 	LPF_2_Value	; Store Potentiometer Value

    call 	EEREAD		; Get LPF Value from EEPROM
    banksel	LPF_3_Value
    movwf 	LPF_3_Value	; Store Potentiometer Value

    call 	EEREAD		; Get LPF Value from EEPROM
    banksel	LPF_4_Value	; Store Potentiometer Value
    movwf 	LPF_4_Value
    ...
    pagesel	Init_DAC_Data	; Init DACs after MAX270 are shutdown.
    call	Init_DAC_Data
...
Loop
    ...
    pagesel	PB_1_Poll
    call	PB_1_Poll	; Test and Count Button for Port L
    call	PB_2_Poll	; Test and Count Button for Port R
    call	PB_3_Poll	; Test and Count Button for Port 1
    call	PB_4_Poll	; Test and Count Button for Port 2

    pagesel	PB_GAIN_Poll
    call	PB_GAIN_Poll	; Test for Gain Button
    pagesel	PB_1PF_Poll
    call	PB_1PF_Poll	; Test for LPF Button
...
Gain_Loop
    ...
    btfsc	Gain_LPF_Flags,PB_1
    call	Gain_Adjust_Loop

    pagesel	Gain_Adjust_Loop
    btfsc	Gain_LPF_Flags,PB_2
    call	Gain_Adjust_Loop

    pagesel	Gain_Adjust_Loop
    btfsc	Gain_LPF_Flags,PB_3
    call	Gain_Adjust_Loop

    pagesel	Gain_Adjust_Loop
    btfsc	Gain_LPF_Flags,PB_4
    call	Gain_Adjust_Loop
...
ReceiveSer1
    ...
    pagesel	Command1
    btfsc	CommandFlags,Received1
    goto	Command1

    pagesel	Command2
    btfsc	CommandFlags,Received2
    goto	Command2

    btfsc	CommandFlags,Received3
    goto	Command3

    btfsc	CommandFlags,Received4
    goto	Command4
...
;    goto	LPFCommandMode
    ...
    pagesel	GainCommandMode
    btfsc	CommandFlags,ReceivedX
    goto	GainCommandMode

    pagesel	LPFCommandMode
    btfsc	CommandFlags,ReceivedZ
    goto	LPFCommandMode

    banksel	GainLPFCom_Flags

    pagesel	GainCommand1
    btfsc	GainLPFCom_Flags,Received_Gain_1
    goto	GainCommand1

    pagesel	GainCommand2
    btfsc	GainLPFCom_Flags,Received_Gain_2
    goto	GainCommand2

    pagesel	GainCommand3
    btfsc	GainLPFCom_Flags,Received_Gain_3
    goto	GainCommand3

    pagesel	GainCommand4
    btfsc	GainLPFCom_Flags,Received_Gain_4
    goto	GainCommand4

    pagesel	LPFCommand1
    btfsc	GainLPFCom_Flags,Received_LPF_1
    goto	LPFCommand1

    pagesel	LPFCommand2
    btfsc	GainLPFCom_Flags,Received_LPF_2
    goto	LPFCommand2

    pagesel	LPFCommand3
    btfsc	GainLPFCom_Flags,Received_LPF_3
    goto	LPFCommand3

    pagesel	LPFCommand4
    btfsc	GainLPFCom_Flags,Received_LPF_4
    goto	LPFCommand4
...
;    goto	RecLowerF
    ...
    movf	RXData,W
    xorlw	0x58		;compare with X
    btfsc	STATUS,Z
    goto	RecUpperX

    movf	RXData,W
    xorlw	0x5A		;compare with Z
    btfsc	STATUS,Z
    goto	RecUpperZ
...
RecUpperX
    banksel	CommandFlags
    bsf		CommandFlags,ReceivedX
    return
RecUpperZ
    banksel	CommandFlags
    bsf		CommandFlags,ReceivedZ
    return

ComRec1
    banksel	CommandFlags
    bsf		CommandFlags,Received1
    bcf		CommandFlags,ReceivedC
    return

ComRec2
    banksel	CommandFlags
    bsf		CommandFlags,Received2
    bcf		CommandFlags,ReceivedC
    return

ComRec3
    banksel	CommandFlags
    bsf		CommandFlags,Received3
    bcf		CommandFlags,ReceivedC
    return

ComRec4
    banksel	CommandFlags
    bsf		CommandFlags,Received4
    bcf		CommandFlags,ReceivedC
    return

GainComRec1
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_Gain_1
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedG
    bcf		CommandFlags,ReceivedX
    return

GainComRec2
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_Gain_2
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedG
    bcf		CommandFlags,ReceivedX
    return
...
GainComRec3
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_Gain_3
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedG
    bcf		CommandFlags,ReceivedX
    return
...
GainComRec4
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_Gain_4
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedG
    bcf		CommandFlags,ReceivedX
    return
...
LPFComRec1
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_LPF_1
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedF
    bcf		CommandFlags,ReceivedZ
    return

LPFComRec2
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_LPF_2
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedF
    bcf		CommandFlags,ReceivedZ
    return
...
LPFComRec3
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_LPF_3
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedF
    bcf		CommandFlags,ReceivedZ
    return
...
LPFComRec4
    banksel	GainLPFCom_Flags
    bsf		GainLPFCom_Flags,Received_LPF_4
    banksel	CommandFlags
    ;bcf		CommandFlags,ReceivedF
    bcf		CommandFlags,ReceivedZ
    return
...
GainCommand1
    ...
    bcf 	Flags,ReceivedX
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_Gain_1
...
GainCommand2
    ...
    bcf 	Flags,ReceivedX
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_Gain_2
    return
...
GainCommand3
    ...
    bcf 	Flags,ReceivedX
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_Gain_3
    return
...
GainCommand4
    ...
    bcf 	Flags,ReceivedX
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_Gain_4
    return
...
LPFCommand1
    ...
    bcf 	Flags,ReceivedZ
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_LPF_1
...
LPFCommand2
    ...
    bcf 	Flags,ReceivedZ
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_LPF_2
...
LPFCommand3
    ...
    bcf 	Flags,ReceivedZ
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_LPF_3
...
LPFCommand4
    ...
    bcf 	Flags,ReceivedZ
    banksel	GainLPFCom_Flags
    bcf 	GainLPFCom_Flags,Received_LPF_4
...
ErrSerialFrame
    ...
    banksel	RCSTA
    bcf		RCSTA,SPEN		; Disable serial port to clear error
...
Inc1
    ...
    pagesel	Mux_1
    call	Mux_1		; Change MUX port

    pagesel	EEPROM_1_Write
...
Inc2
    ...
    pagesel	Mux_2
    call	Mux_2		; Change MUX port

    pagesel	EEPROM_2_Write
...
Inc3
    banksel	Port3Count
    incf	Port3Count,F	; Button was down and now it's up,
				; we need to increment the counter
    bcf		STATUS,Z
    movf	Port3Count,W	; Move L Port Count to W
    xorlw	INPORTNUM	; Is the Port Count equal the
    btfsc	STATUS,Z	;  to the max port number
    clrf	Port3Count	;  if Yes clear port count

    pagesel	Mux_3
    call	Mux_3		; Change MUX port

    pagesel	EEPROM_3_Write
    call	EEPROM_3_Write	; Write port count to EEPROM

    return

Inc4
    banksel	Port4Count
    incf	Port4Count,F	; Button was down and now it's up,
				; we need to increment the counter
    bcf		STATUS,Z
    movf	Port4Count,W	; Move L Port Count to W
    xorlw	INPORTNUM	; Is the Port Count equal the
    btfsc	STATUS,Z	;  to the max port number
    clrf	Port4Count	;  if Yes clear port count

    pagesel	Mux_4
    call	Mux_4		; Change MUX port

    pagesel	EEPROM_4_Write
    call	EEPROM_4_Write	; Write port count to EEPROM
...
LCDLine2
    ...
    pagesel	Dis3Name
    call	Dis3Name	; Display L port Name

    pagesel	DisOutInDiv
    call	DisOutInDiv	; Disp Out In Divider

    banksel	Port3Count
    movf	Port3Count,W	; Move Port Indix to W
    pagesel	DisInName
    call	DisInName	; Display Input Name
    ...
    pagesel	Dis4Name
    call	Dis4Name	; Display R port Name

    pagesel	DisOutInDiv
    call	DisOutInDiv	; Disp Out In Divider

    banksel	Port4Count
    movf	Port4Count,W	; Move Port Indix to W
    pagesel	DisInName
    call	DisInName	; Display Input Name
...
PB_3_Poll
    ...
    movwf	Buttons
				...
				; Check button state
    btfss	PB_State,PB_3	; Was button down?
    goto	PB_3_wasDown	; Yes

PB_3_wasUp
    btfsc	Buttons,PB_3	; Is button still up?
    return			; Was up and still up, do nothing
    bcf		PB_State,PB_3	; Was up, remember now down
    return

PB_3_wasDown
    btfss	Buttons,PB_3	; If it is still down
    return			; Was down and still down, do nothing
    bsf		PB_State,PB_3	; remember released
...
PB_4_Poll
    ...
    movwf	Buttons
				...
				; Check button state
    btfss	PB_State,PB_4	; Was button down?
    goto	PB_4_wasDown	; Yes

PB_4_wasUp
    btfsc	Buttons,PB_4	; Is button still up?
    return			; Was up and still up, do nothing
    bcf		PB_State,PB_4	; Was up, remember now down
    return

PB_4_wasDown
    btfss	Buttons,PB_4	; If it is still down
    return			; Was down and still down, do nothing
    bsf		PB_State,PB_4	; remember released
...
EEPROM_3_Write
    ...
    movlw	EE_3		; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_4_Write
    ...
    movlw	EE_4		; Set the EEPROM address
    movwf	EEADRL
...
PB_1PF_Poll
    ...
    banksel	PB_State	; Check button state
    btfss	PB_State,PB_1PF	; Was button down?
    goto	PB_1PF_wasDown	; Yes

PB_1PF_wasUp
    banksel	Buttons
    btfsc	Buttons,PB_1PF_BIT  ; Is button still up?
    return			; Was up and still up, do nothing
    banksel	PB_State
    bcf		PB_State,PB_1PF	; Was up, remember now down
    return

PB_1PF_wasDown
    banksel	Buttons
    btfss	Buttons,PB_1PF_BIT  ; If it is still down
    return			; Was down and still down, do nothing
    banksel	PB_State
    bsf		PB_State,PB_1PF	; remember released
...
Gain_LPF_PB_1_Poll
    ...
    banksel	PB_State	; Check button state
    btfss	PB_State,PB_1	; Was button down?
    goto	Gain_LPF_PB_1_wasDown	; Yes
...
Gain_LPF_PB_1_wasDown
    ...
    btfss	Gain_LPF_Flags,PB_1 ; Skip if ON, go if off
    goto	$+3

    btfsc	Gain_LPF_Flags,PB_1 ; Skip if OFF, go if on
    goto	$+3

    bsf		Gain_LPF_Flags,PB_1
    return

    bcf		Gain_LPF_Flags,PB_1
    return
...
Gain_LPF_PB_2_Poll
    ...
    banksel	PB_State	; Check button state
    btfss	PB_State,PB_2	; Was button down?
    goto	Gain_LPF_PB_2_wasDown	; Yes
...
Gain_LPF_PB_2_wasDown
    ...
    btfss	Gain_LPF_Flags,PB_2 ; Skip if ON, go if off
    goto	$+3

    btfsc	Gain_LPF_Flags,PB_2 ; Skip if OFF, go if on
    goto	$+3

    bsf		Gain_LPF_Flags,PB_2
    return

    bcf		Gain_LPF_Flags,PB_2
    return
...
Gain_LPF_PB_3_Poll
    ...
    banksel	PB_State	; Check button state
    btfss	PB_State,PB_3	; Was button down?
    goto	Gain_LPF_PB_3_wasDown	; Yes
...
Gain_LPF_PB_3_wasDown
    ...
    btfss	Gain_LPF_Flags,PB_3 ; Skip if ON, go if off
    goto	$+3

    btfsc	Gain_LPF_Flags,PB_3 ; Skip if OFF, go if on
    goto	$+3

    bsf		Gain_LPF_Flags,PB_3
    return

    bcf		Gain_LPF_Flags,PB_3
    return
...
Gain_LPF_PB_4_Poll
    ...
    banksel	PB_State	; Check button state
    btfss	PB_State,PB_4	; Was button down?
    goto	Gain_LPF_PB_4_wasDown	; Yes
...
Gain_LPF_PB_4_wasDown
    ...
    btfss	Gain_LPF_Flags,PB_4 ; Skip if ON, go if off
    goto	$+3

    btfsc	Gain_LPF_Flags,PB_4 ; Skip if OFF, go if on
    goto	$+3

    bsf		Gain_LPF_Flags,PB_4
    return

    bcf		Gain_LPF_Flags,PB_4
    return
...
Gain_LPF_Poll_LRAB_PB
    pagesel	Gain_LPF_PB_1_Poll
    call	Gain_LPF_PB_1_Poll

    pagesel	Gain_LPF_PB_2_Poll
    call	Gain_LPF_PB_2_Poll

    pagesel	Gain_LPF_PB_3_Poll
    call	Gain_LPF_PB_3_Poll

    pagesel	Gain_LPF_PB_4_Poll
    call	Gain_LPF_PB_4_Poll
    return

DisPortDiv1_Gain_LPF
    banksel	Gain_LPF_Flags	    ; Skip if PB_1 was pressed
    btfss	Gain_LPF_Flags,PB_1 ; If not pressed, Display normal divider.
    movlw	"|"		    ; Set W to Port Div

				    ; Skip if PB_1 was not pressed
    btfsc	Gain_LPF_Flags,PB_1 ; If pressed, Display that L is selected.
    movlw	"*"		    ; Set W to Port Div

    pagesel		LCDletr
    call		LCDletr	; Send Char to LCD

    banksel	Gain_LPF_Flags	    ; Skip if PB_2 was pressed
    btfss	Gain_LPF_Flags,PB_2 ; If not pressed, Display normal divider.
    movlw	"|"		    ; Set W to Port Div

				    ; Skip if PB_2 was not pressed
    btfsc	Gain_LPF_Flags,PB_2 ; If pressed, Display that L is selected.
    movlw	"*"		    ; Set W to Port Div
...
DisPortDiv2_Gain_LPF
    banksel	Gain_LPF_Flags	    ; Skip if PB_1 was pressed
    btfss	Gain_LPF_Flags,PB_3 ; If not pressed, Display normal divider.
    movlw	"|"		    ; Set W to Port Div

				    ; Skip if PB_1 was not pressed
    btfsc	Gain_LPF_Flags,PB_3 ; If pressed, Display that L is selected.
    movlw	"*"		    ; Set W to Port Div
    ...
    banksel	Gain_LPF_Flags	    ; Skip if PB_2 was pressed
    btfss	Gain_LPF_Flags,PB_4 ; If not pressed, Display normal divider.
    movlw	"|"		    ; Set W to Port Div

				    ; Skip if PB_2 was not pressed
    btfsc	Gain_LPF_Flags,PB_4 ; If pressed, Display that L is selected.
    movlw	"*"		    ; Set W to Port Div
...
Inc_Gain_1
    ...
    movf	Gain_1_Value,W	; Move Pot Count to W
    xorlw	POTMIN		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    decf	Gain_1_Value,F	; Button was down and now it's up,

    ; Call subruteen to update the DAC
    movlw	H'00'
    pagesel	Set_DAC_Data
    call	Set_DAC_Data
...
Inc_Gain_2
    ...
    bcf		STATUS,Z	; Test if count is at max value.
    movf	Gain_2_Value,W	; Move Pot Count to W
    xorlw	POTMIN		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    decf	Gain_2_Value,F	; Button was down and now it's up,
...
Inc_Gain_3
    ...
    bcf		STATUS,Z	; Test if count is at max value.
    movf	Gain_3_Value,W	; Move Pot Count to W
    xorlw	POTMIN		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    decf	Gain_3_Value,F	; Button was down and now it's up,
...
Inc_Gain_4
    ...
    bcf		STATUS,Z	; Test if count is at max value.
    movf	Gain_4_Value,W	; Move Pot Count to W
    xorlw	POTMIN		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    decf	Gain_4_Value,F	; Button was down and now it's up,
...
Dec_Gain_1
    ...
    incf	Gain_1_Value,F	; Button was down and now it's up.
...
Dec_Gain_2
    ...
    incf	Gain_2_Value,F	; Button was down and now it's up.
...
Dec_Gain_3
    ...
    incf	Gain_3_Value,F	; Button was down and now it's up.
...
Dec_Gain_4
    ...
    incf	Gain_4_Value,F	; Button was down and now it's up.
...
; could change to one subruten for all EEPROM write
EEPROM_Pot_L_Write
    banksel	Gain_1_Value
    movf	Gain_1_Value,W
    ...
    movlw	EE_1_GAIN	; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_Pot_R_Write
    ...
    movlw	EE_2_GAIN	; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_Pot_A_Write
    ...
    movlw	EE_3_GAIN	; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_Pot_B_Write
    ...
    movlw	EE_4_GAIN	; Set the EEPROM address
    movwf	EEADRL
...
Inc_LPF_1
    ...
    bcf		STATUS,Z	; Test if count is at max value.
    movf	LPF_1_Value,W	; Move Pot Count to W
    xorlw	LPFMAX		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    incf	LPF_1_Value,F	; Button was down and now it's up,
...
Inc_LPF_2
    ...
    bcf		STATUS,Z
    movf	LPF_2_Value,W	; Move Pot Count to W
    xorlw	LPFMAX		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return

    incf	LPF_2_Value,F	; Button was down and now it's up,
				; we need to increment the counter
...
Inc_LPF_3
    ...
    bcf		STATUS,Z
    movf	LPF_3_Value,W	; Move Pot Count to W
    xorlw	LPFMAX		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    incf	LPF_3_Value,F	; Button was down and now it's up,
				; we need to increment the counter
...
Inc_LPF_4
    ...
    bcf		STATUS,Z
    movf	LPF_4_Value,W	; Move Pot Count to W
    xorlw	LPFMAX		; Is the Pot Count equal the
    btfsc	STATUS,Z	; to the max port number
    return			; if Yes, do nothing

    incf	LPF_4_Value,F	; Button was down and now it's up,
...
Dec_LPF_1
    ...
    decf	LPF_1_Value,F	; Button was down and now it's up,
...
Dec_LPF_2
    ...
    decf	LPF_2_Value,F	; Button was down and now it's up,
...
Dec_LPF_3
    ...
    decf	LPF_3_Value,F	; Button was down and now it's up,
...
Dec_LPF_4
    ...
    decf	LPF_4_Value,F	; Button was down and now it's up,
...
EEPROM_LPF_L_Write
    ...
    movlw	EE_1_LPF	; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_LPF_R_Write
    ...
    movlw	EE_2_LPF	; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_LPF_A_Write
    ...
    movlw	EE_3_LPF	; Set the EEPROM address
    movwf	EEADRL
...
EEPROM_LPF_B_Write
    ...
    movlw	EE_4_LPF	; Set the EEPROM address
    movwf	EEADRL
...
Set_DAC_Data
    ...
    bcf		STATUS,Z		    ; Get the Gain Number
    pagesel	Get_Gain_1
    movf	Test,W
    xorlw	H'00'
    btfsc	STATUS,Z		    ; Was it Zero?
...
i2c_Gain_Data
    banksel	Value		; Save Gain value
    movwf	Value

    banksel	Temp
    movwf	Temp		; Save gain value to testing register.

    lsrf	Temp,1		; Left the 4 high bits
    lsrf	Temp,1
    lsrf	Temp,1
    lsrf	Temp,1
    movfw	Temp
    banksel	High_Byte
    movwf	High_Byte
    movlw	H'20'		; Set the third bit High, CLR bit
    addwf	High_Byte

    banksel	Value
    movfw	Value		; Get the Gain Value
    banksel	Temp
    movwf	Temp		; Save Gain Vaule to the testing register.
    ...
    movlw	I2CMultNoBus	; Start the i2c comm.
    pagesel	i2c_SendMultBusCommand
    call	i2c_SendMultBusCommand
    ...
    banksel	Test
    movfw	Test
    pagesel	Get_DAC_Address
    call	Get_DAC_Address

    banksel	SSPBUF		; - Send DAC address
    movwf	SSPBUF		; write is i2c buss
    btfsc	SSPSTAT,R_NOT_W	; Wait r_w clear at 9th (ACk) clock
    goto	$-1
    ...
    banksel	Test
    movfw	Test
    pagesel	Get_DAC_Pointer_Byte
    call	Get_DAC_Pointer_Byte

    banksel	SSPBUF		; - Send DAC Pointer Byte
    movwf	SSPBUF		; write is i2c buss
    btfsc	SSPSTAT,R_NOT_W	; Wait r_w clear at 9th (ACk) clock
    goto	$-1
    ...
    banksel	SSPBUF		; - Send 1st DAC Data Byte
    movwf	SSPBUF		; write is i2c buss
    btfsc	SSPSTAT,R_NOT_W	; Wait r_w clear at 9th (ACk) clock
    goto	$-1
    ...
    banksel	SSPBUF		; - Send 2nd DAC Data Byte
    movwf	SSPBUF		; write is i2c buss
    btfsc	SSPSTAT,R_NOT_W	; Wait r_w clear at 9th (ACk) clock
    goto	$-1
...
Get_DAC_Address
    brw
    dt	AD5337_W1_Addr,AD5337_W1_Addr,AD5337_W2_Addr,AD5337_W2_Addr

Get_DAC_Pointer_Byte
    brw
    dt	H'01',H'02',H'01',H'02'
